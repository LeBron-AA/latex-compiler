...
\newpage
$\romannumeral 5)$ Por definición, $\linf{n}f_n(x) = \sup_{n\in\N} \inf_{k \geq n} f_k(x) = \sup_{n\in\N} g_n(x)$ para $g_n (x)= \displaystyle \inf_{k \geq n} f_k(x)$, que es medible por el apartado anterior. Aplicando el apartado $(\romannumeral 4)$ respecto de las $g_n$ llegamos al resultado.
...
\begin{enumerate}[label=\roman*)]
    \item $\mu_1\Big(\{x \in [a,b] : f \text{ es discontinua en } x\}\Big) = 0$.
...
\end{tcolorbox}
Denotando $D$ al conjunto anterior, tenemos que $\mu(D) = 0 \Rightarrow \mu(D \cup V) = 0$ para $V$ el conjunto de vértices de las particiones $P_n$. De este modo, $s \overset{\mu-a.e.}{=}t \Rightarrow s - t \overset{\mu-a.e.}{=} 0$. Por tanto, aplicando el \hyperref[result:2.1.16]{resultado 2.1.16},
...
\begin{flalign*}
    ...
    &\lim_n \underline{S}(f,P_n) = \lim_n \int_{[a,b]}t_n\,d\mu \overset{\hyperref[result:2.2.7]{TCD}}{=} \int_{[a,b]}t\,d\mu = \int_{[a,b]}f\,d\mu
\end{flalign*}
...
\begin{flalign*}
    \int_a^\infty& f = \lim_{x\to\infty} \int_a^x f = \lim_{n\to\infty} \int_a^{a+n} f \overset{\hyperref[result:2.3.1]{2.3.1}}{=} \lim_{n\to\infty}\mathop{\int}_{[a,a+n]}f\,d\mu_1 = \lim_{n\to\infty} \int_{[a,+\infty)}f \cdot X_{[a,a+n]}\,d\mu_1 \overset{\hyperref[result:2.2.7]{TCD}}{=} \\
...
\vspace{6mm} $\romannumeral 3)$ Para $E = \smallcap_{n\in\N}O^n$ intersección de abiertos decrecientes acotados. Aplicando el apartado anterior, sabemos que para $x \in \R^p$ cualquiera se cumple:
\begin{align*}
...
\begin{adjustwidth}{0.07\textwidth}{}
    \vspace{-5ex}
    \begin{flalign*}
        \text{a) }E_x = G_x \setminus N_x \in \mathfrak{M}_q \text{ por ser } G_x, N_x \in \mathfrak{M}_q&&
    \end{flalign*}
    \\[-10ex]
    \begin{flalign*}
    ...
    \\[-8ex]
    \begin{flalign*}
        \text{c) }& \int_{\R^p \setminus A}\varphi(x)\,d\mu_p(x) = \int_{\R^p \setminus A}0\,d\mu_p(x) = \varphi_1(x) - \varphi_2(x) = \mu_{p+q}(G) - 0 = \mu_{p+q}(E)&&
    \end{flalign*}
\end{adjustwidth}
...
\begin{align*}
    ...
    \int_{\R^p \setminus A^n}\varphi_n(x)\,d\mu_p(x) = \mu_{p+q}(E^n)
\end{align*}
...
\begin{adjustwidth}{0.07\textwidth}{}
    \begin{flalign*}
        \text{a) } f_x = \sum_{i=1}^{k} \lambda_i \cdot \underbracket{X_{E^i_x}}_{\mu_q\text{-medible}} \hspace{1mm} \mu_q \text{-medible}&&
    \end{flalign*}
    \\[-6ex]
    \begin{flalign*}
...
\begin{adjustwidth}{0.07\textwidth}{}
    ...
    \begin{flalign*}
        \int_{X}&|f(x)+g(x)|\,d\mu(x) = \int_{X} \Big|\hspace{1mm} \underbrace{\frac{1}{2}}_\lambda\cdot \hspace{1mm} 2f(x)+ \underbrace{\frac{1}{2}}_{1-\lambda}\cdot \hspace{1mm} 2g(x)\hspace{1mm} \Big|^p\,d\mu(x) = (*)
    \end{flalign*}
...
\begin{adjustwidth}{0.07\textwidth}{}
    ...
    $$\hat{g}(x) := \sum_{n=1}^{\infty}f_{k_{n+1}}(x) - f_{k_n}(x) < \infty$$
...
\begin{enumerate}[label=\roman*)]
    ...
    \end{flalign*}
    Para $n \in \N$ cualquiera, se cumple:
...
$$(*) = ||f||^2 + \sum_{k=0}^{n}\Big[ |b_k-c_k|^2 - |c_k|^2 \Big] = ||f||^2 + ||t_n - s_n||^2 - ||s_n||^2$$
...
Análogamente, la serie de Fourier de $f$ respecto de $\{\varphi_n\}_{n\in\N}$ viene dada por:
...
$$f(t) \sim \frac{1}{2} - \frac{2}{\pi}\sum_{n=0}^\infty \frac{1}{2n+1}\sin\big((2n+1)t\big)$$
...
\result{Teorema de Riesz-Fisher}
\begin{onehalfspace}
  \hspace{3mm} Sea $\{\varphi_n\}_{n\in\N} \subseteq L_2(I,K)$ un sistema ortonormal. Sea $\{c_n\}_{n\in\N} \subseteq K$ que verifica $\smallsum_{n=0}^{\infty}|c_n|^2<\infty$. Entonces, existe una función $f$ de $L_2(I.K)$ que cumple simultáneamente:
\end{onehalfspace}
  \begin{align*}
...
$$||s_p-s_q||^2 = \left|\left|\sum_{k=p+1}^{q}s_k\right|\right|^2 = \sum_{k=p+1}^{q}|c_k|^2 \leq \sum_{k=p+1}^{\infty}|c_k|^2 \xrightarrow{p\to\infty}0$$
...
\begin{flalign*}
    s_n(x) &= \frac{1}{\pi} \int_{0}^{2\pi} f(t) \cdot \frac{\sin\left(\left(n+\frac{1}{2}\right)(x-t)\right)}{2\sin\left(\frac{x-t}{2}\right)} \,dt = \substack{\text{cambio de variable}\\u = x-t} && \\[1ex]
    &= -\frac{1}{\pi} \int_{x}^{x-2\pi} f(x-u)\cdot D_n(u) \,du = \frac{1}{\pi} \int_{0}^{2\pi}f(x-u) \cdot D_n(u) \,du = \\[1ex]
...
\begin{adjustwidth}{0.07\textwidth}{}
  \vspace{2mm} \textit{
  Sean $f \in L_1([a,b], \R)$ cualquiera, $\varepsilon > 0$ cualquiera. Existen $g \in L_1([a,b], \R)$ y \hspace{0.5mm}  $s: [a,b] \longrightarrow \R$ combinación lineal de funciones características de intervalos compactos en $[a,b]$ tales que:} \\[-4ex]
  \begin{align*}
...
\begin{flalign*}
    \lim_{n\to\infty} s_n(x) &= \lim_{n\to\infty} \frac{1}{\pi} \int_{0}^{\delta} \Big(f(x+t) + f(x-t)\Big) \cdot \frac{\sin\big((n+\frac{1}{2})t\big)}{t} \,dt + &\\
    &+ \lim_{n\to\infty}  \frac{1}{\pi} \int_{\delta}^{\pi} \Big(f(x+t) + f(x-t)\Big) \cdot \frac{\sin\big((n+\frac{1}{2})t\big)}{t} \,dt
\end{flalign*}
...
\hspace{3mm} Sea $g : [0,b] \longrightarrow \R$ tal que $\exists \mlim{t\to 0^+} \hspace{1mm} g(t) =: g(0^+)$. Si existe $\delta \in (0,b)$ tal que
...
\begin{flalign*}
    ...
    &= \lim_{\alpha \to \infty} \frac{2}{\pi} \int_{0}^{\delta}g(t) \cdot\frac{\sin(\alpha t)}{t} \,dt = g(0^+) = \lim_{t\to 0^+} \frac{f(x+t) + f(x-t)}{2}
\end{flalign*}
Aplicación directa del \hyperref[result:4.2.6]{Teorema de Dini}.Las hipótesis de este enunciado nos garantizan las condiciones para ello.
...
\begin{flalign*}
    a_0 = \frac{1}{\pi} \int_0^{2\pi} f(t) \,dt = \frac{1}{\pi} \int_0^{2\pi}t\,dt = \frac{1}{\pi} \cdot \frac{2^2 \pi^2}{2} = 2\pi&&
\end{flalign*}
...
Entonces, la serie de Fourier de $f$ converge Cesàro a $s(x)\hspace{2mm} \forall \hspace{1mm} x \in A$.

\vspace{4mm}
Además, si $f$ es continua en $\R$, las sumas de Fejèr ($\sigma_n$) convergen de manera uniforme a $s = f$ en $A = \R$; es decir,
...
\begin{enumerate}[label=\roman*)]
    ...
    $$ ||f||^2 = \sum_{k=0}^{\infty} |c_k|^2 = \frac{a_0^2}{4} + \sum_{n=1}^{\infty} \pi \cdot a_n^2 + \pi \cdot b_n^2$$
    \item Si la serie de Fourier de $f$ converge en $x$, converge a $f(x)$.
    \item La serie de Fourier es integrable término a término:
    $$\int_0^x f(t) \,dt = \frac{a_0}{2}x + \sum_{n=1}^{\infty} \int_0^x a_n \cos(nt) + b_n \sin(nt) \,dt$$
\end{enumerate}
\vspace{4mm}
\dem
\vspace{2mm} \newline \indent
...
\newpage
\begin{tcolorbox}[Subset-contingency]
  \hspace{-2mm} $\romannumeral 3$ \hspace{2mm} Como $f$ es continua en $\R$, es $L_1([0,2\pi],\R)$. Aplicando el \hyperref[result:4.2.13]{Teorema de Fèjer},
\end{tcolorbox}
las sumas $\sigma_n$ convergen uniformemente a $s$, luego
$$\sigma_n (x) \xrightarrow{n\to\infty} s(x) = \lim_{t\to 0^+} \frac{f(x+t)+f(x-t)}{2} = f(x)$$

\vspace{2mm}
\begin{tcolorbox}[Subset-contingency]
  \hspace{-1mm} $\romannumeral 4$ \hspace{2mm} Sea $F(x) = \int_0^x f(t) \,dt$
\end{tcolorbox}
\vspace{-3ex}
\begin{flalign*}
  \hspace{3mm} \text{Sea } g(x) = f(x) - \frac{a_0}{2} = f(x) - \frac{1}{2\pi}\int_0^{2\pi} f(x) \,dt = f(x) - m    &&
\end{flalign*}
Es claro que $g$ es continua y $2\pi$-periódica en $\R$ por serlo $f$. La serie de Fourier de $g$ será:
$$g(x) \sim \sum_{n=1}^{\infty}a_n\cos(nx) + b_n \sin(nx) $$
Donde los $a_n$, $b_n$ coinciden con los coeficientes de Fourier de la serie de $f$.
\begin{flalign*}
    \hspace{3mm} \text{La función } G(x) := \int_0^x g(t) \,dt \text{ es } \mathcal{C}^1(\R) \text{ y } 2\pi\text{-periódica} &&
\end{flalign*}
Su serie de Fourier viene dada por:
$$G(x) \sim \frac{A_0}{2} + \sum_{n=1}^{\infty}A_n \cos(nx) + B_n \sin(nx)$$
Veamos la relación que hay entre $A_n, B_n$ con $a_n, b_n$.
\begin{flalign*}
    A_n &= \frac{1}{\pi} \int_0^{2\pi} G(t) \cos(nt) \,dt = \hspace{3ex} \text{Integración por partes: } \substack{u = G(t) \\ dv = \cos(nt)} &&\\[1ex]
    &= \left[\frac{1}{n\pi}G(t)\sin(nt)\right]_{t=0}^{t=2\pi} - \frac{1}{n\pi} \int_0^{2\pi} g(t) \sin(nt) \,dt =  -\frac{1}{n}b_n
\end{flalign*}
\begin{flalign*}
    B_n &= \frac{1}{\pi} \int_0^{2\pi} G(t) \sin(nt) \,dt = \hspace{3ex} \substack{u = G(t) \\ dv = \sin(nt)} &&\\[1ex]
    &= \left[-\frac{1}{n\pi}G(t)\cos(nt)\right]_{t=0}^{t=2\pi} + \frac{1}{n\pi} \int_0^{2\pi} g(t) \cos(nt) \,dt =  \frac{1}{n}a_n
\end{flalign*}

\newpage
Por tanto, comparando con $F$,
$$F(x) = \int_0^x f(t) \,dt = \int_0^x m\,dt + \int_0^x g(t)\,dt = mx + G(x)$$
Por el \hyperref[result:4.2.13]{Teorema de Fejèr}, como $g$ es continua en $\R$ y con periodo $2\pi$, tenemos que las $\sigma_n^g(t)$ convergen uniformemente a $g(t)$ para todo $t \in \R$. De este modo, al tener convergencia uniforme podemos intercambiar el límite con la integral:
$$G(x) = \int_0^x g(t) \,dt = \int_{0}^{x}\lim_{n\to\infty}\sigma_n^g(t) = \lim_{n\to\infty}\int_0^x \sigma_n^g(t) \,dt$$

\vspace{2mm}
Sea $n \in \N$ cualquiera, $\sigma_n^g = \frac{1}{n}\Big(s_1^g + \ldots + s_n^g\Big)$. Por haber escogido $g$ y no $f$, tenemos que todas las $s_k^g$ tienen su término independiente nulo. Además, para cada $n\in\N$ y para cada $k \in \{1,\ldots,n\}$ se tiene:
$$\sigma_n ^g(t) = \frac{1}{n} \sum_{k=1}^{n} \sum_{i=1}^{k}a_i \cos(ix) +  b_i \sin(ix) = \sum_{k=1}^{n}\alpha_n^{(k)}\cos(kt) + \beta_n^{(k)}\sin(kt)$$
\vspace{2mm} De este modo, para $n \geq k$, tenemos:
\begin{align*}
    \alpha_n^{(k)} = \frac{n+1-k}{n} \cdot a_k \xrightarrow{n\to\infty}a_k &&
    \beta^{(k)} = \frac{n+1-k}{n} \cdot b_k \xrightarrow{n\to\infty}b_k 
\end{align*}

\vspace{2mm}\noindent
Volviendo a la integral de $\sigma_n^g$, para cada $n\in\N$,
$$\int_0^x \sigma_n^g(t) = \sum_{k=1}^{n}\left[ \frac{\alpha_n^{(k)}}{k}\sin(kt) - \frac{\beta_n^{(k)}}{k}\cos(kt)\right]_{t=0}^{t=x} \hspace{-2ex}
=\sum_{k=1}^{n}\left[ \frac{\alpha_n^{(k)}}{k}\sin(kx) - \frac{\beta_n^{(k)}}{k}\cos(kx) +\frac{b^{(k)}}{k} \right]$$
De este modo, tomando límites:
$$\lim_{n\to\infty} \int_{0}^{x} \sigma_n^g(t) \,dt = \sum_{k=1}^{\infty} \frac{a_k}{k} \sin(kx) - \frac{b_k}{k} \cos(kx) + \frac{b_k}{k}$$
Comparando la expresión de las integrales de $f$ y $g$ llegamos al resultado.

\newpage
\result{Convergencia ante diferenciabilidad}
\hspace{3mm} Sea $f \in L_2([0,2\pi],\R)$ una función con periodo $2\pi$ en $\R$. Si $f$ es diferenciable en $x_0$; entonces, su serie de Fourier converge en $x_0$ a $f(x_0)$.

\vspace{4mm} \dem Sin demostración.

\vspace{6mm}
\result{Convergencia ante \texorpdfstring{$\mathcal{C}^2$}{C2}}
\hspace{3mm} Sea $f \in \mathcal{C}^2(\R)$ una función con periodo $2\pi$. Entonces, la serie de Fourier de $f$ converge uniformemente a $f(x)$ para todo $x\in\R$. Esto a su vez implica que la serie de Fourier es derivable término a término.
