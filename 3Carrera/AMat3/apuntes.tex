...
\begin{align*}
    \mu\Big(X \setminus \{x \in X : f(x) > \alpha\} \Big) = 0 &&
    \{x \in X : f(x) > \alpha\} \overset{\mu-a.e.}{=} X
\end{align*}
...
\begin{tikzpicture}
    ...
    \draw[<->, thick] (0,-1) -- (0,\maxY);
    \def\f(#1){exp(sin(1.2*#1 r))}
    \draw[domain=-1:\maxX+1, samples=200, smooth, thick, DeepSkyBlue3]
        plot (\x, {\f(\x)});
\end{tikzpicture}
\end{center}
\end{document}