...
\vspace{2mm}
\dem \newline \indent Sea $A_n = A \cap [-n,n]^N$, tenemos que $(A_n)_n$ es una sucesión creciente de conjuntos medibles que tiene como unión el conjunto $A$. Por tanto, se tiene
...
\vspace{2mm} \noindent $\romannumeral 3)$ Utilizaremos una variación del conjunto de Cantor. En cada iteración ``eliminamos'' la parte central de cada intervalo, hasta quitar $\frac{1}{4^n}$ en total. Veamos ejemplos de algunas iteraciones:
\\[-2ex]
\begin{align*}
  &&\Big[0,1\Big]&&
\end{align*}
...
\end{align*}
En la $n$-ésima iteración eliminamos $2^{n-1}$ de longitud total $\frac{1}{4^n}$. Como los intervalos quitados son disjuntos, podemos calcular su medida total como la suma de las longitudes quitadas. Sea $L_n$ la longitud quitada en la $n$-ésima iteración,
\begin{align*}
  L_n = \frac{1}{4^n} &&
  \sum_{n=1}^\infty L_n = \frac{1}{3}
\end{align*}
Considerando $O = (0,1)\setminus C$ para $C$ la intersección de todas las iteraciones, tenemos que la frontera de $O$ es $C$, que tiene como medida $1 - \frac{1}{3} = \frac{2}{3}$.

\vspace{2mm}
$\romannumeral 4)$ Falso: basta considerar el conjunto de Cantor original.
...
\newpage
\chapter{Aplicaciones del lenguaje de la Teoría de la Medida}
\result{Ejercicio 3}
Teorema de Egorov. Sea \(E\in\mathcal{M}\) tal que \(\mu(E)<\infty\) y \((f_n)\) una sucesión de funciones medibles \(f_n: E\to\mathbb{R}\) que convergen puntualmente a la función \(f:E\to\mathbb{R}\). Demuestra que para todo \(\varepsilon>0\) existe un conjunto medible \(D\subset E\) tal que \(\mu(E\setminus D)<\varepsilon\) tal que la sucesión \((f_n|_D)\) converge uniformemente a \(f|_D\).

Para probar el anterior teorema, sigue los pasos siguientes:
\begin{enumerate}
\item para cada par de números naturales \(i,j\), considera el conjunto
\[
E_{ij}:=\{x\in E:\ |f(x)-f_k(x)|<1/i\ \forall k\ge j\}.
\]
Demuestra que \(E_{ij}\) es medible;
\item para cada \(k\), determina \(\displaystyle\bigcup_{j=1}^{\infty} E_{kj}\);
\item para cada \(k\), encuentra \(i_k\in\mathbb{N}\) verificando \(\mu\bigl(E\setminus E_{k i_k}\bigr)<2^{-k}\varepsilon\);
\item estudia el conjunto
\[
D:=\bigcap_{k=1}^{\infty} E_{k i_k}.
\]
\end{enumerate}

\vspace{4mm} \dem \vspace{2mm} \newline
\textbf{1)} \hspace{3mm} Por ser límite de funciones medibles, $f$ es medible. Así, para cada $n \in \N$ tenemos $f - f_n$ medible, luego $|f - f_n|$ medible al componerse con una función continua. En vista de la definición de función medible, los $E_{i,j}$ son conjuntos medibles.

\vspace{4mm} \noindent
\textbf{2)} \hspace{3mm} Sea $k \in \N$ fijo, veamos $\smallcup_{j\in\N}E_{k,j} = E$. Sea $x \in E$, sabemos por hipótesis que $f_n(x) \xrightarrow{n\to\infty} f(x)$ luego para $\delta = \frac{1}{k}$ tenemos que existe $n_x \in \N$ tal que:
$$ |f(x) - f_n(x)| < \frac{1}{k} \hspace{2mm} \forall \hspace{1mm}n \geq n_x$$
De este modo, $x \in E_{k,n_x} \subseteq \smallcup_{j\in\N}E_{k,j}$.

\vspace{4mm} \noindent
\textbf{3)} \hspace{3mm} Sea $k \in \N$ fijo, por definición de los $E_{i,j}$ se tiene que $E_{k,n} \subseteq E_{k,n+1}$. Así, tenemos que $(E_{k,n})_{n\in\N}$ es una colección creciente de conjuntos medibles cuya unión es el propio $E$. Como $\mu(E) < \infty$ por hipótesis, podemos despejar adecuadamente:

$$\mu(E \setminus E_{k,n}) = \mu(E) - \mu(E_{k,n})  \xrightarrow{n\to\infty} 0$$
Por definición de límite, podemos tomar $i_k \in \N$ tal que $\mu(E \setminus E_{k,i_k}) < 2^{-k} \varepsilon$.

\vspace{4mm} \noindent
\textbf{4)} \hspace{3mm} Tenemos que $D$ es medible por ser intersección numerable de conjuntos medibles. Además, su medida queda acotada por:
$$\mu(E \setminus D) = \mu(E \setminus (\smallcap_{k\in\N}E_{k,i_k})) = \mu\left(\cup_{i\in\N}(E \setminus E_{k,i_k})\right) \leq \sum_{k=1}^\infty \mu(E \setminus E_{k,i_k}) < \sum_{k=1}^\infty \frac{\varepsilon}{2^k} = \varepsilon$$

Veamos que $f_n$ converge uniformemente en $D$. Sea $\delta > 0$, existe $n_\delta \in \N$ tal que $\frac{1}{n} < \delta$. Así, sea $x \in D \subseteq E_{n_\delta, i_{n_\delta}}$, se cumple $|f_n(x) - f(x)| < \delta$ para cualquier $n \geq i_{n_\delta}$, independientemente de $x$.

\newpage
\result{Ejercicio 5}
\hspace{3mm} Sea $f : [a,b] \longrightarrow \R$ una función integrable Riemann tal que $f(x) > 0$ $\forall \hspace{1mm} x \in [a,b]$. Demuestra que $\int_{a}^{b}f(x) dx > 0$.

\vspace{4mm} \dem \vspace{2mm} \newline \indent
Sabemos que toda función integrable Riemann en un intervalo compacto está acotada. Por tanto, aplicando el resultado 2.3.1 de los apuntes de teoría, sabemos que $\mu(A) = 0$ para el siguiente conjunto:
$$ A := \{x \in [a,b] : f \text{ no es continua en } x\}$$
Además, al ser $f \in \mathcal{R}([a,b])$, la integral de Riemann coincide con la de Lebesgue, luego:
$$ \int_a^b f(x) \,dx = \int_A f \,d\mu + \int_{[a,b] \setminus A}f \,d\mu$$
Por ser $[a,b] \setminus A$ un conjunto medible de medida finita positiva, sabemos que contiene a un compacto $K$ con $\mu(K) > 0$ (ejercicio 2 de conjuntos medibles). De este modo, $f$ es continua en $K$, luego alcanza un mínimo $m$. Como $f > 0$ en $K$, $m > 0$. Definimos la función simple $s := \frac{m}{2} \cdot X_K$ luego $0 \leq s \leq f$ con $s$ función medible simple. Así,
$$\lbint_{[a,b]} s \,d\mu = \frac{m}{2} \cdot \mu(K)> 0$$
Así, por definición de Integral de Lebesgue, $\int_{[a,b]}f\,d\mu \geq \lbint_{[a,b]} s > 0$.
...
\vspace{6mm} \result{Ejercicio 4}
\hspace{3mm} Dada una función con dominio $A \subseteq \R^N$ y llegada $\overline{\R}$, demuestra que si es continua en $x_0 \in A$; entonces, $f^+$ y $f^-$ también son continuas en $x_0$.

\vspace{4mm} \dem \vspace{2mm} \newline \indent
Supongamos en primer lugar que $f(x_0) \in (0, \infty)$. De ser así, para $\varepsilon = \frac{|f(x_0)|}{2}$ existe cierto $\delta > 0$ tal que $||x-x_0||< \delta \Rightarrow f(x) \in \left(\frac{f(x_0)}{2}, \frac{3f(x_0)}{2}\right) \subseteq (0, +\infty)$.

\vspace{2mm}
De este modo, en $B_{||\cdot||}(x_0, \delta) \subseteq A$, tenemos que $f^+ = f$, $f^- \equiv 0$, ambas continuas. En el caso $f(x) = \infty$ tendríamos $f(x) > K$ para cierto $K > 0$ fijo en lugar de trabajar con un error de $\varepsilon$. Este razonamiento también funciona para tal escenario. El caso $f(x) \in [-\infty, 0)$ se resuelve análogamente.

\vspace{4mm} Si $f(x_0) = 0$, para $\varepsilon > 0$ cualquiera existe $\delta > 0$ tal que:
$$||x-x_0|| < \delta \Rightarrow |f(x)| < \varepsilon$$
Como $|f| = f^+ + f^-$ siendo ambos sumandos no negativos, esto implica que $f^+$ y $f^-$ son continuas al cumplir (para $x \in B_{||\cdot||}(x_0,\delta)$):
\begin{flalign*}
    |f^+(x) - f^+(x_0)| = f^+(x) \leq |f(x)| < \varepsilon \\[1ex]
    |f^-(x) - f^-(x_0)| = f^-(x) \leq |f(x)| < \varepsilon    
\end{flalign*}
...
\end{align*}
\begin{flalign*}
  \int_{0}^\delta \frac{1}{t} \cdot \frac{1}{\log\left(\frac{1}{t}\right)} = \overset{u = \log\left(\frac{1}{t}\right)}{=} \int_{\infty}^{\log\left(\frac{1}{\delta}\right)} \hspace{-2mm} -e^{-u} \frac{1}{e^{-u}} \cdot\frac{1}{u} \,d\mu = \int^{\infty}_{\log\left(\frac{1}{\delta}\right)} \frac{1}{u} \,d\mu = \infty
\end{flalign*}
La integral diverge, luego no se cumple la condición de Dini.
