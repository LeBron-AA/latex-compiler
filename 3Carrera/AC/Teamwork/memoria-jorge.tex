...
% ---------------------------------------------------------
% SECCIÓN ORIGINAL DEL DOCUMENTO
% ---------------------------------------------------------
...
\subsection{Fase 2 – Multihilo (10 hilos → 2×5 procesadores)}
...
A partir de los tiempos medios obtenidos para cada una de las tres implementaciones, calculamos la aceleración relativa frente a la versión monohilo, definida como:

\[
S = \frac{T_{\text{monohilo}}}{T_{\text{implementación}}}
\]
...
\begin{itemize}
    ...
    \]
    Esto implica que la versión multihilo es aproximadamente un 59.8\% más rápida que la versión secuencial.

    \item \textbf{Aceleración del SIMD frente al monohilo}:
    \[
    S_{\text{SIMD}} = \frac{8.972}{9.488} \approx \mathbf{0.95}
    \]
    El resultado indica que la versión SIMD es un 5\% más lenta que la versión monohilo en este caso concreto.
\end{itemize}

Estos valores muestran que, para nuestro algoritmo de inversión en blanco y negro, la paralelización mediante hilos ofrece una mejora significativa del rendimiento, mientras que la versión SIMD no logra superar al enfoque monohilo debido al pequeño volumen de operaciones por píxel y a la sobrecarga asociada a las instrucciones vectoriales.


\newpage
\section{Comparativa visual de los resultados}
...
\centering
%\includegraphics[width=\textwidth]{media/antes_monohilo.png}
\caption*{Imagen original}
\end{minipage}
\hfill
\begin{minipage}{0.45\textwidth}
\centering
%\includegraphics[width=\textwidth]{media/despues_monohilo.png}
...
\begin{minipage}{0.45\textwidth}
\centering
%\includegraphics[width=\textwidth]{media/antes_simd.png}
\caption*{Imagen original}
\end{minipage}
\hfill
\begin{minipage}{0.45\textwidth}
\centering
%\includegraphics[width=\textwidth]{media/despues_simd.png}
\caption*{Resultado tras aplicar el algoritmo}
\end{minipage}
...
\begin{minipage}{0.45\textwidth}
\centering
%\includegraphics[width=\textwidth]{media/antes_multihilo.png}
\caption*{Imagen original}
\end{minipage}
\hfill
\begin{minipage}{0.45\textwidth}
\centering
%\includegraphics[width=\textwidth]{media/despues_multihilo.png}
\caption*{Resultado tras aplicar el algoritmo}
\end{minipage}
...
En esta sección se muestran las capturas de la herramienta \texttt{diffImages}, utilizada para comparar pixel a pixel las imágenes generadas por las distintas implementaciones del algoritmo. Esta utilidad permite comprobar visualmente si existen diferencias entre los resultados producidos por cada variante del programa.

\subsection{\texttt{diffImages}: SIMD vs Monohilo}

En esta figura se incluye la ejecución de la aplicación \texttt{diffImages} comparando la imagen resultante del programa SIMD con la generada por la versión monohilo. Idealmente, al tratarse del mismo algoritmo matemático, la diferencia debe ser completamente nula.

\begin{figure}[H]
\centering
%\includegraphics[width=0.85\textwidth]{media/diff_simd_vs_monohilo.png}
\caption{Ejecución de \texttt{diffImages} comparando SIMD con Monohilo}
\end{figure}

\subsection{\texttt{diffImages}: Multihilo vs Monohilo}

A continuación, se muestra la comparación entre la salida del programa multihilo y la del programa monohilo. Nuevamente, al implementar exactamente la misma operación sobre cada píxel, el resultado debería ser idéntico.

\begin{figure}[H]
\centering
%\includegraphics[width=0.85\textwidth]{media/diff_multihilo_vs_monohilo.png}
\caption{Ejecución de \texttt{diffImages} comparando Multihilo con Monohilo}
\end{figure}
% ---------------------------------------------------------
\end{document}