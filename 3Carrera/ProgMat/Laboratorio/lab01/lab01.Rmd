...
a[1:3]
```
Podemos usar la función \texttt{seq} de R para generar vectores siguiento el formato "start, step, stop":
```{r}
...
solve(A) %*% b
```
En la última línea, \texttt{solve(A)} halla la matriz inversa de \texttt{A}.Finalmente, hacemos el producto de matrices con `%*%`.
