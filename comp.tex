...
$$\int_{X}f\,d\mu = \sup\left\{\lbint_{X}s\,d\mu \hspace{2mm} \text{ con } 0 \leq s \leq s, \hspace{1mm} s \text{ función simple medible}\right\} $$
...
\end{cases} $$
Sabemos por Análisis Matemático II que esta función es integrable-Riemann en $[0,1]$ con valor de su integral igual a $0$. Veamos que ocurre lo mismo con la integral de Lebesgue. Sea $s$ una función simple medible tal que $0 \leq s \leq f$. Se cumple $M := \max s([0,1]) \leq 1$. Los puntos donde $s$ toma valores positivos son a lo sumo numerables,
%TODO acabar (continúa al final de la línea anterior)
\end{document}